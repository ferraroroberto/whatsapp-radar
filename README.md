...
A FastAPI + vanilla-JS admin PWA runs on port **8455**, mirroring App Launcher's auth/tunnel model: a bearer token (loopback bypasses it), an optional login password, WebAuthn passkeys (enrolled from the tray, ceremonies Tailscale-only), Tailscale TLS, and dormant Cloudflare named-tunnel scaffolding. Of the four tabs — Dashboard · Chats & Config · Execution · Audit — all are live. The **Dashboard** shows read-only metrics (channels monitored, messages stored, scans run, backlog since the last scan, alerts raised, notifications sent, plus a per-monitored-channel table — a linked family is folded into its parent as one row whose count and last-activity span the whole family) served by `GET /api/dashboard`. **Chats & Config** lets you pick which chats are watched (a searchable Monitored/Ignored/All list ordered by last activity, a single watch toggle per row, and a tap-to-open conversation overlay that pages older messages in as you scroll up; marking a chat monitored baselines its review cursor to only new messages). From the overlay you can also ✏️ **rename** a chat — an operator alias that shows first with the connector-derived name in parentheses (e.g. `Tom (+44123…)`), the human fallback for chats WhatsApp only exposes as a bare number. The 🔗 **link** button merges the same person reached under two different numbers (old/new SIM, work/personal) into one family: open the chat that should fold in, search by name or alias for the canonical parent, and link it as a child. Linked children drop out of the chat list (the parent shows a `🔗N` badge), the parent's overlay shows a time-ordered **merged history** across the family with each message tagged by origin, and monitoring/review/digest treat the family as one subject so the same person is never watched or alerted twice. Linking is pure local metadata — reversible (unlink restores the child), moves no message data, and each member keeps its own review cursor; it is manual only (no auto-suggested duplicates). It also lets you inspect the classifier: the LLM system prompt and keyword roots are shown read-only (edited in their `src/analysis/prompts/` files by design), while the safe settings subset (connector, classifier, notifier, hub model — persisted to `config/local.json`; Telegram token/chat-id masked and persisted to `config/webapp_config.json`) is editable. Scan frequency stays in App Launcher's Jobs tab. Endpoints: `GET /api/chats`, `GET /api/chats/{id}/history`, `POST /api/chats/{id}/status`, `POST /api/chats/{id}/alias`, `POST /api/chats/{id}/link`, `POST /api/chats/{id}/unlink`, `GET`/`POST /api/config`. **Execution** runs the pipeline — whole or in pieces — and streams it live, mirroring App Launcher's job-run view: each action spawns the matching `launcher.py` command as a subprocess and tails its combined output beside a parsed funnel. Pick the steps to run (Sync · Process · Message) and a Live/Dry-run mode (dry-run simulates the whole pipeline on stored data — no sync, no delivery; live composes the ticked steps, where Message means "send"), then watch the run: a funnel (synced → monitored-with-delta → Stage 1 → Stage 2 LLM → actionable → notification status), the would-be/sent Telegram message, and the live output log, with a recent-runs history and a WhatsApp **connection health** dot (the sidecar's pairing/heartbeat). When the connection is down the health pill grows a one-tap **Reconnect** that relaunches the sidecar, and — when the device needs re-linking — shows the **pairing QR right in the phone UI** (served no-cache from `GET /api/sidecar/qr`) so a non-technical household member can re-pair from their phone without a terminal (`GET /api/sidecar/status`, `POST /api/sidecar/start`). A **Recent syncs** card lists each ingest — *timestamp · chats added · messages added* — with the current stored totals, written to a `sync_log` row by every sync path (the webapp Sync button, a scheduled `wr resync` Job, or a live scan) so you can confirm at a glance that new messages are landing; the per-message ingest time lives on `messages.ingested_at` (`GET /api/execution/syncs`). A **Maintenance** card offers **Sync** (incremental, idempotent upsert from the connector buffer) and a guarded **Rebuild** (full cache rebuild for after a reader-logic change — backs up the DB, preserves monitored/ignored/alias state and parent↔child links across the canonical re-key, resets run history). Runs are single-flight (one at a time, sharing the SQLite store). Endpoints: `POST /api/execution/run`, `GET /api/execution/runs`, `GET /api/execution/runs/{kind}/{id}`, `POST /api/execution/runs/{kind}/{id}/kill`, `GET /api/execution/health`. **Audit** is the read-only trust surface over the persisted per-run trace: a list of every review/scan run (live vs dry-run, its parameters, and the funnel counters), most recent first, with resync/reprocess maintenance events interleaved so the whole timeline is visible. Drilling into a run shows, per chat, the complete decision record — a **per-message breakdown** of every message analyzed (each with the Stage-1 keyword roots it matched, or none, and — when the LLM ran — whether the model flagged it as actionable evidence), the **exact** LLM system+user prompts sent, the **raw** model response, the parsed verdict, the final action, and the Telegram text it contributed — so you can confirm exactly which messages triggered and which didn't, and why any message was or wasn't promoted (the radar's real failure mode is a silently-missed important message). When a run synced messages but none landed in a monitored chat, the drill-down says so explicitly rather than showing a bare "no trace". Endpoints: `GET /api/audit/runs`, `GET /api/audit/runs/{id}`.
