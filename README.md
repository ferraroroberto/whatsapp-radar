...
- `GET /api/family` (rules + recent runs), `POST /api/family` (the full editable schedule + toggles + threshold, validated)
...
- `family-radar-calendar-sync` — `wr calendar-scan`, daily at 18:05 (the family calendar-conflict summary, #168). Deliberately its own job on its own schedule rather than chained after the scan job — a dead WhatsApp sidecar must never suppress the family calendar summary.
