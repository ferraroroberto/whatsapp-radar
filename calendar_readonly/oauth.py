"""Standalone installed-app OAuth bootstrap for the read-only Calendar scope."""
...
from calendar_readonly.google_client import write_token_atomically
...
def authorize(
    *,
    ...
    flow = flow_loader(str(credentials_path), [CALENDAR_READONLY_SCOPE])
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        logger.info("ℹ️ Opening Google consent for read-only Calendar access.")
        authorize(
        ...
        logger.error("❌ %s", exc)
        return 1
    logger.info("✅ Calendar read-only token stored at %s", args.token)
