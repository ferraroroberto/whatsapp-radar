...
**Only voice-note audio is downloaded** (#36) — every other media type stays text-only (caption or placeholder), read-only and privacy-preserving. For a voice note the sidecar downloads the encrypted audio (`downloadMediaMessage`, a read-only fetch — not a send/reaction/receipt) to the ignored `data/linked_device/media/`, tags the row `transcription_status='pending'` + `media_path`, and keeps the `[voice note]` placeholder. The Python core's transcription phase then turns it into real text (see the README's "Voice-note transcription" section); on success the transcript overwrites `text`, the placeholder is preserved in `raw_json`, and the audio file is **retained on disk for `audio_retention_days` days** (default 7, set in `config/default.json`) so it can be played back in the Chats overlay; a sweep at the start of each transcription phase then deletes audio past that window and clears `media_path`. Set `audio_retention_days: 0` to revert to deleting the audio immediately on success (the original #36 behaviour). WhatsApp delivers voice notes as OGG/Opus and the hub's whisper backend only decodes WAV, so the transcription client transcodes each note to 16 kHz mono WAV with **ffmpeg** before sending (verified against the live hub: it 400s on raw OGG).
...
6. **How does offline catch-up behave after the service is stopped for several hours?** On reconnect, Baileys delivers the messages received while offline (subject to WhatsApp's history window). Because storage dedupes idempotently and cursors only track what was analysed, catch-up simply appears as a larger delta on the next review. **Empirical recovery proof (2026-07-19):** a metadata-only query over the local run/message store found six consecutive offline scans from 2026-06-20 through 2026-06-25; the next connected scan (run 30) ingested 552 WhatsApp messages whose source timestamps fell inside that outage window. No message text or identity was read by the probe. This verifies multi-day catch-up for that deployment and outage, not an unlimited guarantee: a freshly paired device or an outage beyond WhatsApp's available history can still have a bounded-history gap.
