...
1. Create a connector prototype behind a narrow interface: connect, report status, list chats, ingest events, stop.
2. Persist raw-but-local message records into SQLite with stable internal IDs and source message IDs.
3. Add a sanitized fixture connector so storage, cursoring, analysis, and reporting can be developed without WhatsApp access.
4. Build a CLI command that prints discovered chats with IDs and sanitized labels.
5. Build a CLI command that marks chats as monitored in local SQLite.
6. Build a CLI command that reviews monitored chats since the last cursor and emits a dry-run JSON digest.
7. Call local-llm-hub only after the fixture path and cursoring are proven.
8. Add Telegram delivery only after the digest JSON is stable.
