...
#: The modules that build a family run payload.
_PAYLOAD_MODULES = (
...
def _payload_keys() -> set[str]:
    """Every string key the payload builders construct in a dict literal."""
    keys: set[str] = set()
...
def test_every_payload_key_is_classified() -> None:
    ...
    unclassified = _payload_keys() - renderable - _WITHHELD_ON_PURPOSE - _NOT_IN_THE_RUN_PAYLOAD
